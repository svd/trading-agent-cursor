...
- `user-markethub-mcp/get_stock_history` - historical data for trend analysis. Use **only** `start_date` and `end_date` (YYYY-MM-DD). Do not use period.
...
- `user-markethub-mcp/get_crypto_history` - historical data. Use **only** `start_date` and `end_date` (YYYY-MM-DD). Do not use period.
...
- Args:
  ...
  - `start_date`, `end_date` (string) - YYYY-MM-DD format. **Use these only; do not use period.**
