...
> **Примечание:** Это правило было конвертировано в skill `homework`. Для активации используйте skill `@trading-homework`. Это правило оставлено для справки.
...
### 1. Скрининг инструментов (см. @trading-trends)

**Цель**: Найти инструменты, подходящие для торговли сегодня.

**ВАЖНО:** При анализе трендов, фаз рынка и силы тренда используй детальные правила из @trading-trends.
...
- **Фильтр чёткого сетапа**: Строго применять фильтр — торговать только инструменты с "супер понятной картинкой" (очевидный сетап, чёткие уровни, однозначный тренд, ясный вход)
...
**Общие критерии** (см. @trading-trends):
- **Глобальный тренд (W1/MN)**: Определить общее направление рынка на старших таймфреймах (см. @trading-trends)
- **Локальный тренд (D1/H1)**: Чёткий тренд на D1 или H4, либо инструмент у границы диапазона (см. @trading-trends)
- **Сила тренда**: Оценить силу тренда (ценовое действие + объемы + ATR) (см. @trading-trends)
- **Фаза рынка**: Определить текущую фазу (накопление/импульс/коррекция/ретест/флэт/истощение) — избегать инструментов во флэте в середине диапазона (см. @trading-trends)
...
- **Для акций**: Используй `get_stock_history` для анализа тренда на разных таймфреймах (W1, D1, H1) (см. @trading-trends)
  - `server`: `"user-markethub-mcp"`
  - `toolName`: `"get_stock_history"`
  - `arguments`: Используй **только** `start_date` и `end_date` (формат YYYY-MM-DD). Параметр period не использовать. Для анализа уровней — 6 месяцев данных, для графиков — 5 месяцев. Пример: `{"symbol": "AAPL", "interval": "1d", "start_date": "YYYY-MM-DD", "end_date": "YYYY-MM-DD"}`

- **Для криптовалют**: Используй `get_crypto_history` для анализа тренда на разных таймфреймах (W1, D1, H1) (см. @trading-trends)
  - `server`: `"user-markethub-mcp"`
  - `toolName`: `"get_crypto_history"`
  - `arguments`: Используй **только** `start_date` и `end_date` (формат YYYY-MM-DD). Параметр period не использовать. Пример: `{"symbol": "BTCUSDT", "interval": "1d", "start_date": "YYYY-MM-DD", "end_date": "YYYY-MM-DD", "exchange": "binance"}` (exchange опционально)
...
**ВАЖНО:** При определении уровней используй детальные правила из @trading-levels. Ниже приведено краткое резюме ключевых моментов.
...
#### Приоритетные типы уровней (см. @trading-levels для деталей)
...
#### Оценка силы уровня (см. @trading-levels для детальных критериев)
...
**Ключевой сигнал:** Отсутствие реакции на сильный уровень — самый важный сигнал пробоя (см. @trading-levels для деталей)

#### Фильтрация уровней

**После определения всех уровней:**
1. Оценить силу каждого уровня по критериям из @trading-levels
...
### 3. Проверка предпосылок (см. @trading-trends)

**ВАЖНО:** При проверке тренда и фазы рынка используй детальные правила из @trading-trends.
...
- [ ] **Глобальный тренд (W1/MN)**: Определен и синхронизирован с локальным (см. @trading-trends)
- [ ] **Локальный тренд (D1/H1)**: Инструмент в тренде ИЛИ у границы диапазона (см. @trading-trends)
- [ ] **Сила тренда**: Оценена (ценовое действие + объемы + ATR) (см. @trading-trends)
- [ ] **Фаза рынка**: Определена (не флэт в середине диапазона) (см. @trading-trends)
- [ ] **Уровень**: Есть чёткий уровень для работы (не более 0.5% от текущей цены)
- [ ] **Волатильность**: ATR(5) достаточен для движения к TP (минимум 2x от риска) (см. @trading-trends)
- [ ] **Объём**: Объём выше среднего (минимум 1.2x от 20-дневного среднего, желателен контрастный объем 2-3x) (см. @trading-trends)
...
### 4. Формирование торгового сценария (см. @trading-trends)

Для каждого инструмента, прошедшего проверку, создать торговый сценарий:

**ВАЖНО:** При формировании торгового сценария обязательно учитывать информацию из @trading-trends (глобальный/локальный тренд, сила тренда, фаза рынка, ATR(5) для стопов, расчетный стоп = 20-30% от ATR).

**ВАЖНО:** При формировании торгового сценария используй принципы торговли на уровнях из @trading-levels, особенно принцип близости (торговля максимально близко к уровню).
...
1. **Направление**: Long или Short (на основе глобального и локального тренда, см. @trading-trends)
2. **Триггер входа**: Чёткое условие (например, "Пробой 185.50 с ретестом и подтверждением объемом") (см. @trading-trends)
3. **Вход**: Конкретная цена или условие
   - **Принцип близости:** Вход должен быть максимально близко к уровню (см. @trading-levels)
...
4. **Стоп-лосс** (см. @trading-trends): 
   - Не более 1% от депозита на сделку
   - **Приоритет технического стопа:** Предпочитать технический стоп (за уровнем), если он не превышает расчетный (20-30% от ATR(5)) (см. @trading-trends)
   ...
   - Рассчитать расчетный стоп: 20-30% от ATR(5) (например, если ATR = $1.00, то расчетный стоп = $0.20 - $0.30) (см. @trading-trends)
   - Минимум 0.2% от цены инструмента
   - Минимум 0.5 ATR от входа (избежать рыночного шума) (см. @trading-trends)
...
## Торговый сценарий (см. @trading-trends)
