...
- **Для акций**: Используй `get_stock_history` для анализа тренда на разных таймфреймах (W1, D1, H1) (см. @trends)
  ...
  - `arguments`: `{"symbol": "AAPL", "interval": "1d", "period": "6mo"}` или с `start_date` и `end_date`
...
- **Тикер**: Всегда ссылка на файл с домашкой по тикеру в формате `[SYMBOL](./SYMBOL.md)`. Файл должен находиться в той же папке `homework/YYYY-MM-DD/`
- **Рекомендация**: 
...
3. **Настройки графика**:
   ...
   - **title**: Опционально, можно указать тикер и дату (например, "AAPL - 2026-01-25")

**Расположение файла графика**: `homework/YYYY-MM-DD/SYMBOL.png` (или `.jpg` в зависимости от формата, возвращаемого инструментом)

**Процесс генерации графика**:

1. **Подготовить данные**: Использовать исторические данные OHLCV и уровни из анализа
2. **Вызвать MCP инструмент**: Использовать `call_mcp_tool` с параметрами:
   ...
   - `arguments`: объект с полем `input_data`, содержащим:
     ...
     - `ohlcv`: массив исторических данных OHLCV
     - `levels`: массив уровней в формате `ChartLevel`
     - `chart_type`: `"candle"`
     - `style`: `"light"` (или `"dark"`)
     - `renderer`: `"mplfinance"` (или `"plotly"`)
     - `title`: опционально, название графика
3. **Сохранение графика**: 
   - MCP инструмент возвращает base64-encoded изображение
   - Декодировать base64 и сохранить изображение в файл `homework/YYYY-MM-DD/SYMBOL.png`
   - Добавить ссылку на график в файл с домашкой по инструменту

**Добавление графика в файл домашнего задания**:

В файл `homework/YYYY-MM-DD/SYMBOL.md` добавить раздел с графиком:
...
- Используй те же исторические данные, что и для анализа уровней (D1, последние 6 месяцев)
